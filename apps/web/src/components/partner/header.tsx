...
export function PartnerHeader() {
    ...
    const pathSegments = pathname.split("/").filter(Boolean).slice(1) // remove 'partner'
    ...
    // Mock Notifications
    const notifications = [
        { id: 1, title: "Welcome!", message: "Welcome to your new dashboard", time: "Just now", unread: true },
    ]
    ...
    return (
        <header className="sticky top-0 z-40 w-full mb-8">
            <div className="flex items-center justify-between py-4">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 text-sm text-white/40 overflow-hidden">
                        <Link href="/partner/dashboard" className="hover:text-white transition-colors shrink-0">
                        ...
                        {pathSegments.map((segment, i) => (
                            <div key={i} className="flex items-center gap-2 overflow-hidden">
                                ...
                                <Link
                                    href={`/partner/${pathSegments.slice(0, i + 1).join('/')}`}
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="relative" ref={notifRef}>
                        ...
                        {isNotificationsOpen && (
                            <div className="absolute right-0 top-12 w-80 bg-[#0A0A0A] border border-white/10 rounded-xl shadow-2xl p-0 overflow-hidden backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="relative" ref={profileRef}>
                        ...
                        >
                            ...
                            <span className="text-sm font-medium text-white hidden md:block">Partner</span>
                            <ChevronDown className={`w-3 h-3 text-white/40 transition-transform ${isProfileOpen ? 'rotate-180' : ''}`} />
                        </button>

                        {/* Dropdown Menu */}
                        {isProfileOpen && (
                            <div className="absolute right-0 top-12 w-56 bg-[#0A0A0A] border border-white/10 rounded-xl shadow-2xl p-1 overflow-hidden backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200">
                                <Link
            ...
            <MobileSidebar isOpen={isMobileMenuOpen} onClose={() => setIsMobileMenuOpen(false)}>
                <PartnerSidebar className="relative h-full w-full border-none" onNavigate={() => setIsMobileMenuOpen(false)} />
            </MobileSidebar>
        </header>
    )
}
