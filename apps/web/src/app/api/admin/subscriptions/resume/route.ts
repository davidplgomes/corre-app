...
export async function POST(request: Request) {
    try {
        ...
        const updatePayload: Record<string, string | boolean> = {
            cancel_at_period_end: stripeJson.cancel_at_period_end ?? false,
            status: nextStatus,
            updated_at: new Date().toISOString(),
        };

        if (typeof stripeJson.current_period_start === 'number') {
            updatePayload.current_period_start = new Date(stripeJson.current_period_start * 1000).toISOString();
        }
        if (typeof stripeJson.current_period_end === 'number') {
            updatePayload.current_period_end = new Date(stripeJson.current_period_end * 1000).toISOString();
        }
